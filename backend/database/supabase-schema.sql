-- Complete Supabase Schema for Epic Quiz App
-- This includes the main schema plus the chapter_summaries table for our content generation pipeline
...
CREATE TABLE questions (
    ...
    -- Cultural context and source reference
...
-- Chapter Summaries: Generated summaries from our content pipeline
-- This table stores the chapter summaries generated by our LLM content service
CREATE TABLE chapter_summaries (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    epic_id VARCHAR(50) NOT NULL REFERENCES epics(id) ON DELETE CASCADE,
    
    -- Chapter identification
    kanda VARCHAR(50) NOT NULL,
    ...
    -- Unique constraint per epic/kanda/sarga combination
    CONSTRAINT unique_chapter UNIQUE (epic_id, kanda, sarga)
);
...
-- Chapter summaries lookup
CREATE INDEX idx_chapter_summaries_epic_kanda ON chapter_summaries(epic_id, kanda);
CREATE INDEX idx_chapter_summaries_epic_sarga ON chapter_summaries(epic_id, kanda, sarga);
...
CREATE TRIGGER update_chapter_summaries_updated_at BEFORE UPDATE ON chapter_summaries
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
